...
You can execute the `ListProjectsForClient` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
listProjectsForClient(vars: ListProjectsForClientVariables): QueryPromise<ListProjectsForClientData, ListProjectsForClientVariables>;
...
```typescript
listProjectsForClient(dc: DataConnect, vars: ListProjectsForClientVariables): QueryPromise<ListProjectsForClientData, ListProjectsForClientVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the listProjectsForClientRef:
```typescript
const name = listProjectsForClientRef.operationName;
console.log(name);
```
...
```
### Return Type
Recall that executing the `ListProjectsForClient` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `ListProjectsForClientData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `ListProjectsForClient`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, listProjectsForClient, ListProjectsForClientVariables } from '@dataconnect/generated';

// The `ListProjectsForClient` query requires an argument of type `ListProjectsForClientVariables`:
const listProjectsForClientVars: ListProjectsForClientVariables = {
  clientId: ..., 
};

// Call the `listProjectsForClient()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await listProjectsForClient(listProjectsForClientVars);
// Variables can be defined inline as well.
const { data } = await listProjectsForClient({ clientId: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await listProjectsForClient(dataConnect, listProjectsForClientVars);

console.log(data.projects);

// Or, you can use the `Promise` API.
listProjectsForClient(listProjectsForClientVars).then((response) => {
  const data = response.data;
  console.log(data.projects);
});
```

### Using `ListProjectsForClient`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, listProjectsForClientRef, ListProjectsForClientVariables } from '@dataconnect/generated';
...
// Call the `listProjectsForClientRef()` function to get a reference to the query.
const ref = listProjectsForClientRef(listProjectsForClientVars);
// Variables can be defined inline as well.
const ref = listProjectsForClientRef({ clientId: ..., });

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = listProjectsForClientRef(dataConnect, listProjectsForClientVars);
...
## AddProjectTask
You can execute the `AddProjectTask` mutation using the following action shortcut function, or by calling `executeMutation()` after calling the following `MutationRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
addProjectTask(vars: AddProjectTaskVariables): MutationPromise<AddProjectTaskData, AddProjectTaskVariables>;
...
```typescript
addProjectTask(dc: DataConnect, vars: AddProjectTaskVariables): MutationPromise<AddProjectTaskData, AddProjectTaskVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the addProjectTaskRef:
```typescript
const name = addProjectTaskRef.operationName;
console.log(name);
```
...
```
### Return Type
Recall that executing the `AddProjectTask` mutation returns a `MutationPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `AddProjectTaskData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
export interface AddProjectTaskData {
  projectTask_insert: ProjectTask_Key;
}
```
### Using `AddProjectTask`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, addProjectTask, AddProjectTaskVariables } from '@dataconnect/generated';

// The `AddProjectTask` mutation requires an argument of type `AddProjectTaskVariables`:
...
// Call the `addProjectTask()` function to execute the mutation.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await addProjectTask(addProjectTaskVars);
// Variables can be defined inline as well.
const { data } = await addProjectTask({ assigneeId: ..., projectId: ..., dueDate: ..., name: ..., status: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await addProjectTask(dataConnect, addProjectTaskVars);

console.log(data.projectTask_insert);

// Or, you can use the `Promise` API.
addProjectTask(addProjectTaskVars).then((response) => {
  const data = response.data;
  console.log(data.projectTask_insert);
});
```

### Using `AddProjectTask`'s `MutationRef` function

```typescript
import { getDataConnect, executeMutation } from 'firebase/data-connect';
import { connectorConfig, addProjectTaskRef, AddProjectTaskVariables } from '@dataconnect/generated';
...
// Call the `addProjectTaskRef()` function to get a reference to the mutation.
const ref = addProjectTaskRef(addProjectTaskVars);
// Variables can be defined inline as well.
const ref = addProjectTaskRef({ assigneeId: ..., projectId: ..., dueDate: ..., name: ..., status: ..., });

// You can also pass in a `DataConnect` instance to the `MutationRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = addProjectTaskRef(dataConnect, addProjectTaskVars);
